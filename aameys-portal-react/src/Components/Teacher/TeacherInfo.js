...
export default class TeacherInfo extends Component {
    ...
    logout(){
        ...
        axios.get('http://localhost:5000/updateteacherstatus?id='+this.state.teacher_id+"&status="+0).
        then(res =>{
            if(res.status < 300){
                // this.props.history=[]
    ...
    componentDidMount(){
        ...
        .then(res =>{
            console.log(res)
            this.setState({
                teacher_name_last: res.data[0]['last_name'],
                teacher_name_first: res.data[0]['first_name'],
                teacher_mail: res.data[0]['email']
            })
        })
    }
    onChange(e) {
